...
document.addEventListener("DOMContentLoaded", () => {
	...
	function patchToList() {
		for (const release in patchFiles) {
			const listVersion = document.createElement("ul")
			listVersion.setAttribute("class", "release")
			listVersion.innerHTML = `
			<div class="category">${release}</div>`

			const versionList = document.createElement("ul")
			for (const patch in patchFiles[release]) {
				const patchVer = patchFiles[release][patch]
				const valuePatch = document.createElement("li")
				valuePatch.setAttribute("id", patchVer)
				...
				<div class="version">${patchVer}</div>`
	...
	function loadPatch() {
		...
		while (patchNoteField.firstChild) {
			patchNoteField.removeChild(patchNoteField.firstChild);
		}
